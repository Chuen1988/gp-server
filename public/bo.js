...
const socket = io('https://192.168.50.232:2007', {
  ...
  <div class="card w-full max-w-sm bg-base-100 px-4 py-8 shadow-xl">
    ...
    <form class="card-body pt-2">
      ...
      <div class="form-control">
        <label for="userNameBO" class="label"><span class="label-text">Username BO</span></label>
        <input type="text" name="userNameBO" placeholder="Enter username bo" class="input input-bordered">
      </div>
      ...
      <div class="form-control mt-0">
        ...
        <input type="text" name="company" placeholder="Enter company" class="input input-bordered">
      </div>
      ...
      userList.innerHTML += `<li class="user">
        <a>
          <div class="avatar indicator">
            <div class="w-6 rounded"><img src="logo.png"/></div>
          </div><span>${user?.userName ? "MC - " + user?.userName : user?.userNameBO ? "BO - " + user?.userNameBO : user?.contactNumber }</span>
        </a>
      </li>`
    ...
    if (chat) {
      chat.innerHTML += `<div class="chat chat-start py-2">
        ...
        <div class="chat-header pb-1">
          ${message?.user?.userName ? "MC - " + message?.user?.userName : message?.user?.userNameBO ? "BO - " + message?.user?.userNameBO : message?.user?.contactNumber }
  ...
  // Retrieve userNameBO/password/company object from the login/signup page
  const getCredentials = () => {
    const user = {
      userNameBO: document.querySelector('[name="userNameBO"]').value,
  ...
  // Log in either using the given userNameBO/password/company or the token from storage
